...
//#define sendData(packet, length)  	(varSerial->write(packet, length);while(availableData()){readData()})    	// Write Over Serial
...
void AX12A::sendData(uint8_t* packet, uint8_t length)
{
	varSerial->write(packet, length);
